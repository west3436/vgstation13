...
mod tests {
    ...
    impl TileBuilder {
        ...
        fn plasma(mut self, value: f32) -> Self {
            self.0.gases.set_plasma(value);
            self
        }
        ...
        fn cryotheum(mut self, value: f32) -> Self {
            self.0.gases.set_cryotheum(value);
            self
        }
        fn volatile_fuel(mut self, value: f32) -> Self {
            self.0.gases.set_volatile_fuel(value);
            self
        }
        fn oxygen_agent_b(mut self, value: f32) -> Self {
            self.0.gases.set_oxygen_agent_b(value);
            self
        }
    ...
    struct TileChecker {
		...
		cryotheum_: Option<f32>,
		volatile_fuel_: Option<f32>,
        oxygen_agent_b_: Option<f32>,
    ...
    impl TileChecker {
        ...
        fn check(self, tile: &Tile, x: i32, y: i32) {
            ...
            if let Some(value) = self.plasma_ {
                assert!(
                    (tile.gases.plasma() - value).abs() < TEST_TOLERANCE,
                    "{} != {} @ ({}, {})",
                    tile.gases.plasma(),
                    value,
                    x,
            ...
            if let Some(value) = self.cryotheum_ {
                assert!(
                    (tile.gases.cryotheum() - value).abs() < TEST_TOLERANCE,
                    "{} != {} @ ({}, {})",
                    tile.gases.cryotheum(),
                    value,
                    x,
                    y
                );
            }
            if let Some(value) = self.volatile_fuel_ {
                assert!(
                    (tile.gases.volatile_fuel() - value).abs() < TEST_TOLERANCE,
                    "{} != {} @ ({}, {})",
                    tile.gases.volatile_fuel(),
                    value,
                    x,
                    y
                );
            }
            if let Some(value) = self.oxygen_agent_b_ {
                assert!(
                    (tile.gases.oxygen_agent_b() - value).abs() < TEST_TOLERANCE,
                    "{} != {} @ ({}, {})",
                    tile.gases.oxygen_agent_b(),
                    value,
                    x,
                    y
                );
        ...
        fn cryotheum(mut self, value: f32) -> Self {
            self.cryotheum_ = Some(value);
            self
        }
        fn volatile_fuel(mut self, value: f32) -> Self {
            self.volatile_fuel_ = Some(value);
            self
        }
        fn oxygen_agent_b(mut self, value: f32) -> Self {
            self.oxygen_agent_b_ = Some(value);
            self
        }
        fn thermal_energy(mut self, value: f32) -> Self {
