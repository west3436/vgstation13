...
pub(crate) fn check_interesting(
    ...
    if (my_inactive_tile.gases.plasma() >= PLASMA_VISIBILITY_MOLES)
        != (my_tile.gases.plasma() >= PLASMA_VISIBILITY_MOLES)
    {
        // Crossed the plasma visibility threshold.
...
pub(crate) fn react(my_inactive_tile: &mut Tile) -> f32 {
    ...
    if my_inactive_temperature > AGENT_B_CONVERSION_TEMP
        && is_significant(my_inactive_tile, GAS_OXAGENT)
        && is_significant(my_inactive_tile, GAS_CARBON)
        && is_significant(my_inactive_tile, GAS_PLASMA)
    {
        let co2_converted = (my_inactive_tile.gases.carbon_dioxide() * 0.75)
            .min(my_inactive_tile.gases.plasma() * 0.25)
            .min(my_inactive_tile.gases.oxygen_agent_b() * 0.05);
        ...
        my_inactive_tile
            .gases
            .set_oxygen_agent_b(my_inactive_tile.gases.oxygen_agent_b() - co2_converted * 0.05);
        // Recalculate existing thermal energy to account for the decrease in heat capacity
        // caused by turning very high capacity plasma into much lower capacity carbon
        // dioxide.
    ...
    if my_inactive_temperature > SLEEPING_GAS_BREAKDOWN_TEMP
        && is_significant(my_inactive_tile, GAS_SLEEPING)
    {
        ...
        && is_significant(my_inactive_tile, GAS_OXYGEN)
    {
        ...
        my_inactive_tile
            .gases
            .set_plasma(my_inactive_tile.gases.plasma() - plasma_burnt);
        my_inactive_tile
            .gases
