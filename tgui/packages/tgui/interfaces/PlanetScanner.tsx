...
// Sub-component for Access Denied display
const AccessDeniedSection = () => (
  <Stack.Item grow>
    <Section fill>
      <Stack fill vertical justify="center" align="center">
        <Stack.Item>
          <Box fontSize="48px" color="bad" bold textAlign="center" mb={2}>
            ⚠ ACCESS DENIED ⚠
          </Box>
        </Stack.Item>
        <Stack.Item>
          <Box fontSize="20px" color="bad" textAlign="center" mb={1}>
            DEEP SPACE SCANNING DISABLED
          </Box>
        </Stack.Item>
        <Stack.Item>
          <Box fontSize="14px" color="label" textAlign="center" style={{ maxWidth: '400px' }}>
...
// Sub-component for Scanner Status
...
const ScannerStatusSection = ({ available_power, min_power_rate, at_scan_limit, scans_completed, max_scans }: ScannerStatusProps) => (
  <Stack.Item>
    <Section title="Scanner Status">
      <Stack vertical>
        <Stack.Item>
          <Stack>
            <Stack.Item basis="40%">Available Power:</Stack.Item>
            <Stack.Item grow>
              <ProgressBar
...
// Sub-component for Scan Filters
type ScanFiltersProps = {
...
const ScanFiltersSection = ({
  act, has_filters, filtered_heat, filtered_humidity, filtered_terrain,
  filtered_atmosphere, filter_cost_multiplier, estimated_scan_time, possible_planets,
}: ScanFiltersProps) => (
  <Stack.Item>
    <Section
      title="Scan Filters"
      buttons={
        <Button icon="times" content="Clear Filters" disabled={!has_filters} onClick={() => act('clear_filters')} />
      }
    >
      <Stack vertical>
        <Stack.Item>
        ...
        {possible_planets && possible_planets.length > 0 && (
          <Stack.Item>
            <Box fontSize="11px" color="label" mt={0.5}>{possible_planets.join(', ')}</Box>
          </Stack.Item>
        )}
        {possible_planets && possible_planets.length === 0 && (
          <Stack.Item>
            <Box fontSize="11px" color="bad" mt={0.5}>No planets match these filters!</Box>
          </Stack.Item>
        )}
      </Stack>
    </Section>
...
// Sub-component for Generation Progress
type GenerationProgressProps = {
  generation_stage: number | null;
  generation_progress: number | null;
};

const GenerationProgressSection = ({ generation_stage, generation_progress }: GenerationProgressProps) => {
  const stage = generation_stage ?? 0;
  const progress = generation_progress ?? 0;
      ...
      <Section title="Generating Planet...">
        <Stack vertical>
          <Stack.Item>
            ...
            <ProgressBar
              value={stage >= STAGE_TERRAIN ? (stage > STAGE_TERRAIN ? 100 : progress) : 0}
              maxValue={100}
              color={stage > STAGE_TERRAIN ? "good" : stage === STAGE_TERRAIN ? "average" : "default"}
            />
          </Stack.Item>
          <Stack.Item>
            <Box mb={0.5}>Classifying Flora & Fauna</Box>
            <ProgressBar
              value={stage >= STAGE_POPULATION ? (stage > STAGE_POPULATION ? 100 : progress) : 0}
              maxValue={100}
              color={stage > STAGE_POPULATION ? "good" : stage === STAGE_POPULATION ? "average" : "default"}
            />
          </Stack.Item>
          <Stack.Item>
            <Box mb={0.5}>Writing data to memory</Box>
            <ProgressBar
              value={stage >= STAGE_WEATHER ? 100 : 0}
              maxValue={100}
              color={stage >= STAGE_FINALIZE ? "good" : stage >= STAGE_WEATHER ? "average" : "default"}
            />
          </Stack.Item>
        </Stack>
      </Section>
...
// Sub-component for Discovered Planets
type DiscoveredPlanetsProps = {
  act: (action: string, payload?: object) => void;
...
const DiscoveredPlanetsSection = ({
  act, currentPlanet, currentPlanetIndex, totalPlanets, goToPreviousPlanet, goToNextPlanet,
}: DiscoveredPlanetsProps) => (
  <Stack.Item grow>
        ...
        <Stack.Item grow>
          <Stack vertical fill>
            ...
            {currentPlanet && currentPlanet.beacons && currentPlanet.beacons.length > 0 && (
              <Stack.Item>
                <Box mb={1} fontSize="14px" bold>Active Trackers:</Box>
                {currentPlanet.beacons.map((beacon, index) => (
                  <Box key={index} fontSize="12px" color={beacon.active ? "bad" : "label"} bold={beacon.active} mb={0.5}>
            ...
            <Stack.Item>
              <Stack>
                <Stack.Item>
                  <Button icon="chevron-left" content="Previous" disabled={totalPlanets <= 1} onClick={goToPreviousPlanet} />
                </Stack.Item>
                <Stack.Item>
                  <Button icon="chevron-right" content="Next" disabled={totalPlanets <= 1} onClick={goToNextPlanet} />
                </Stack.Item>
                <Stack.Item grow />
                <Stack.Item>
                  <Button
...
const ScanButtonSection = ({
  act, scanning, at_scan_limit, can_scan, possible_planets,
  other_scan_in_progress, required_energy, filter_cost_multiplier, estimated_scan_time,
}: ScanButtonProps) => {
  const getButtonContent = () => {
    if (scanning) return "Scanning...";
    if (at_scan_limit) return "Maximum scans reached";
    if (possible_planets?.length === 0) return "No planets match filters";
    return "Start Planet Scan";
  };

  const getTooltip = () => {
    if (other_scan_in_progress) return "A planet scan is already in progress on this station - multiple scans are disabled due to electrical infetterence.";
    if (at_scan_limit) return "Maximum scans reached";
    if (possible_planets?.length === 0) return "No planets match the current filters";
    return `Requires ${required_energy?.toLocaleString() || 0} J (${filter_cost_multiplier?.toFixed(2) || 1}x base) | Est: ${formatTime(estimated_scan_time)}`;
  };
...
export const PlanetScanner = () => {
  const { act, data } = useBackend<Data>();
  const {
    anchored, powered, scanning, scans_completed, max_scans, progress, required_energy,
    min_power_rate, available_power, can_scan, at_scan_limit, discovered_planets,
    has_discoveries, waiting_for_generation, generation_stage, generation_progress,
    other_scan_in_progress, filtered_heat, filtered_humidity, filtered_terrain,
    filtered_atmosphere, filter_cost_multiplier, possible_planets, estimated_scan_time,
    has_filters, scanning_disabled,
  } = data;
  ...
  const goToPreviousPlanet = () => {
    if (totalPlanets > 0) {
      setCurrentPlanetIndex((prev) => prev > 0 ? prev - 1 : totalPlanets - 1);
    }
  };

  const goToNextPlanet = () => {
    if (totalPlanets > 0) {
      setCurrentPlanetIndex((prev) => prev < totalPlanets - 1 ? prev + 1 : 0);
    }
  };
    ...
    <Window width={600} height={620}>
      <Window.Content>
        <Stack fill vertical>
          ...
          {!!scanning_disabled && !!powered && !!anchored && <AccessDeniedSection />}

          {!!powered && !!anchored && !scanning_disabled && (
            <>
              <ScannerStatusSection
                available_power={available_power}
                min_power_rate={min_power_rate}
                at_scan_limit={at_scan_limit}
              ...
              {!scanning && !waiting_for_generation && (
                <ScanFiltersSection
                  ...
                  filtered_humidity={filtered_humidity}
                  filtered_terrain={filtered_terrain}
                  filtered_atmosphere={filtered_atmosphere}
                  filter_cost_multiplier={filter_cost_multiplier}
                  estimated_scan_time={estimated_scan_time}
                  possible_planets={possible_planets}
                />
              )}
              ...
              {!!has_discoveries && !scanning && !waiting_for_generation && (
                <DiscoveredPlanetsSection
                  ...
                  goToPreviousPlanet={goToPreviousPlanet}
                  goToNextPlanet={goToNextPlanet}
                />
              )}
              ...
              <ScanButtonSection
                ...
                can_scan={can_scan}
                possible_planets={possible_planets}
                other_scan_in_progress={other_scan_in_progress}
