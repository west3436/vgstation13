...
/obj/machinery/media/advjukebox
...
/obj/machinery/media/advjukebox/Topic(href, href_list)
	if(!isAdminGhost(usr))
		return
	var/admin_key_name = key_name(usr)
	if(href_list["cancel"])
		if(!current_track)
			to_chat(usr, "<span class='info'>[src] is not playing")
			return
		stop_music()
	...
	if(href_list["removequeue"])
		...
		if(!queue.Find(track))
			if(current_track == track)
				...
				to_chat(usr, "<span class='info'>The track has already been removed.")
		else
			queue -= track
	...
	if(href_list["deny"])
		...
		if(!requests.Find(track))
			to_chat(usr, "<span class='info'>The request has already approved/denied/discarded.")
			return
		requests -= track
		message_admins("[admin_key_name] denied [mob_key_name]'s web sound request [track.webpage_url_html].")
		qdel(track)

/obj/machinery/media/advjukebox/process()
	if(current_track && world.time - track_started_at > current_track.duration)
		skip_music()

/obj/machinery/media/advjukebox/update_icon()
	icon_state = "[initial(icon_state)][is_playing() ? "-active" : null]"
	return ..()

/obj/machinery/media/advjukebox/attackby(obj/item/I, mob/user)
	if(iswrench(I))
		if(busy || is_playing())
...
/obj/machinery/media/advjukebox/ui_status(mob/user)
	if(!ytdl)
...
/obj/machinery/media/advjukebox/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "ElectricalJukebox", name)
		ui.open()

/obj/machinery/media/advjukebox/ui_data(mob/user)
	var/list/data = list()
	if(current_track)
	...
	data["banned"] = juke_isbanned(user) ? TRUE : FALSE
	data["user_ckey"] = user.key
	data["queue"] = list()
...
/obj/machinery/media/advjukebox/ui_act(action, list/params)
	. = ..()

	if(. || busy || !usr.can_perform_action(src, FORBID_TELEKINESIS_REACH))
		return

	if(juke_isbanned(user))
...
/obj/machinery/media/advjukebox/proc/add_queue(mob/user, input)
	if(busy)
		return
...
/obj/machinery/media/advjukebox/proc/new_request(mob/user, input)
	if(busy)
		return
...
/obj/machinery/media/advjukebox/proc/get_web_sound_(input, mob_name, mob_ckey, mob_key_name)
	if(!ytdl)
...
/obj/machinery/media/advjukebox/proc/get_web_sound(input, mob_name, mob_ckey, mob_key_name)
	busy = TRUE
...
/obj/machinery/media/advjukebox/proc/play_in_mob_list(datum/web_track/track)
...
/obj/machinery/media/advjukebox/proc/stop_in_mob_list()
	for(var/mob/user in listeners_in_range)
		if(user.client)
...
/obj/machinery/media/advjukebox/proc/play_music_by_track(datum/web_track/track, looped = FALSE)
...
/obj/machinery/media/advjukebox/proc/play_music_by_input(mob/user, input)
	if(busy || is_playing() || !anchored || !is_operational)
		return
...
/obj/machinery/media/advjukebox/proc/stop_music()
	track_started_at = 0
	QDEL_NULL(current_track)
	stop_in_mob_list()
	update_icon_state()

/obj/machinery/media/advjukebox/proc/skip_music(manually = FALSE)
	if(loop && !manually)
		track_started_at = 0
		stop_in_mob_list()
...
/obj/machinery/media/advjukebox/proc/is_playing()
	if(current_track && world.time - track_started_at < current_track.duration)
		return TRUE
	return FALSE

/obj/machinery/media/advjukebox/proc/can_mob_use(mob/user)
	if(owner == user || HAS_TRAIT(user, TRAIT_CAN_USE_JUKEBOX))
		return TRUE
	return FALSE

/obj/machinery/media/advjukebox/proc/track_unavailable(mob/user, reason)
...
/obj/machinery/media/advjukebox/proc/tgui_input_music(title)
...
/obj/machinery/media/advjukebox/on_set_is_operational(old_value)
	. = ..()
	if(!is_operational)
		stop_music()
