...
/turf/simulated/hotspot_expose(exposed_temperature, exposed_volume, soh)
	...
	if(active_hotspot)
		if(soh)
			if(air.plasma() > 0.5 && air.oxygen() > 0.5)
	...
	if(exposed_temperature > PLASMA_MINIMUM_BURN_TEMPERATURE && air.oxygen() > 0.5 && air.plasma() > 0.5)
		var/total = air.total_moles()
		if(air.plasma() < 0.01 * total || air.oxygen() < 0.01 * total)
...
/obj/effect/hotspot
	...
	icon_state = "1"
	layer = SINGULARITY_LAYER //placeholder
	plane = EFFECTS_PLANE //placeholder
	alpha = 250
...
/datum/milla_safe/hotspot_burn_plasma/on_run(obj/effect/hotspot/hotspot)
	...
	if(location_air.plasma() < 0.5 || location_air.oxygen() < 0.5)
		// Burn what, exactly?
		qdel(hotspot)
		return

	var/total = location_air.total_moles()
	if(location_air.plasma() < 0.01 * total || location_air.oxygen() < 0.01 * total)
		// The rest of the gas is snuffing out the reaction.
		qdel(hotspot)
		return
	...
	var/fuel_burnt = old_toxins - burning.plasma()
...
/obj/effect/hotspot/process()
	...
	if(location.blocks_air || location_air.plasma() < 0.5 || location_air.oxygen() < 0.5)
		qdel(src)
		return
...
/proc/fireflash(atom/center, radius, temp)
	...
	for(var/turf/T in view(radius, get_turf(center)))
		...
		if(isfloor(T) && issimulatedturf(T))
