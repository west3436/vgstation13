...
/turf/proc/GetAtmosAdjacentTurfs(alldir = 0)
	...
	for(var/turf/T in RANGE_TURFS(1, src))
		...
		for(var/check_direction in cardinal)
...
/turf/simulated/proc/atmos_spawn_air(flag, amount)
	...
	if(flag & MILLA_SPAWN_20C)
		G.set_temperature(T20C)

	if(flag & MILLA_SPAWN_HEAT)
		G.set_temperature(G.temperature() + 1000)

	if(flag & MILLA_SPAWN_COLD)
		G.set_temperature(TCMB)

	if(flag & MILLA_SPAWN_TOXINS)
		G.set_plasma(G.plasma() + amount)

	if(flag & MILLA_SPAWN_OXYGEN)
		G.set_oxygen(G.oxygen() + amount)

	if(flag & MILLA_SPAWN_CO2)
		G.set_carbon_dioxide(G.carbon_dioxide() + amount)

	if(flag & MILLA_SPAWN_NITROGEN)
		G.set_nitrogen(G.nitrogen() + amount)

	if(flag & MILLA_SPAWN_N2O)
		G.set_sleeping_agent(G.sleeping_agent() + amount)

	if(flag & MILLA_SPAWN_AGENT_B)
		G.set_agent_b(G.agent_b() + amount)

	if(flag & MILLA_SPAWN_AIR)
