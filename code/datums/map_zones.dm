// A map zone is a collection of one or more virtual z levels residing physically on a single real z level.
/datum/map_zone
	var/name = "Map Zone"
	var/id
	...
	/// Climate controller for this level
	var/datum/climate/climate_controller
...
/datum/map_zone/proc/assert_climate_controller()
	if(!climate_controller)
		new /datum/climate(src)
...
// A virtual z level is a space enclosed by void. Many of these can exist on the same physical zlevel and they can be linked in a map zone to provide z directionality.
/datum/virtual_level
...
/datum/virtual_level/proc/reserve_margin(margin)
	...
	for(var/side in 1 to 4)
		...
		for(var/turf/Turf as anything in block(beginning, ending))
			Turf.ChangeTurf(/turf/unsimulated/wall/edge)
			CHECK_TICK
...
/datum/virtual_level/proc/unlink(direction)
	...
	var/reversed_dir = opposite_dirs[direction]
...
/datum/virtual_level/proc/clear_dir_linkage(direction)
	...
	for(var/turf/unsimulated/wall/edge/edgy_turf as anything in turfblock)
...
/datum/virtual_level/proc/link_with(direction, datum/virtual_level/other_zone)
	...
	var/nb_direction = opposite_dirs[direction]
	if(crosslinked["[direction]"])
		unlink(direction)
	...
	for(var/width_i in 1 to min_transition)
		for(var/height_i in 1 to min_margin)
			...
			var/turf/unsimulated/wall/edge/cur_turf = locate(cur_x, cur_y, z_value)
			var/turf/cur_mirage_turf = locate(cur_mirage_x, cur_mirage_y, z_value)

			var/turf/unsimulated/wall/edge/nb_cur_turf = locate(nb_cur_x, nb_cur_y, other_zone.z_value)
...
/datum/virtual_level/proc/clear_reservation()
	...
	var/area/space_area = new /area

	var/list/turf/block_turfs = get_block()

	var/static/list/ignored_atoms = typecacheof(list(/mob/dead, /atom/movable/lighting_overlay))
	...
	for(var/turf/turf as anything in block_turfs)
		// Reset turf
		turf.ChangeTurf(RESERVED_TURF_TYPE)
		// Reset area
...
/datum/virtual_level/proc/get_alive_client_mobs()
	return LAZYACCESS(SSmob.players_by_virtual_z, "[z_value]") || list()

/datum/virtual_level/proc/get_dead_client_mobs()
	return LAZYACCESS(SSmob.dead_players_by_virtual_z, "[z_value]") || list()

/datum/virtual_level/proc/get_mind_mobs()
	. = list()
	for(var/mob/living/living_mob as anything in living_mob_list)
		if(!living_mob.mind)
			continue
...
/turf/unsimulated/wall/edge/attack_ghost(mob/dead/observer/user)
	if(destination_z)
